...
#
import sys, wave, re, os, struct # Python 3: audioop was removed in 3.13
import numpy as np
...
#helpers:
def AscDec(data, LittleEndian=False):#Converts a byte string into a decimal
	if isinstance(data, str):
		data = data.encode("latin-1")
	return int.from_bytes(bytes(data), "little" if LittleEndian else "big")

def DecAsc(dec, length=None, LittleEndian=False):#Converts a decimal into bytes of chosen length
	if dec < 0:
...
# Python 3.13 removed audioop. This is the IMA/DVI ADPCM decoder used by
# audioop.adpcm2lin(..., 2, None), retaining the original initial state.
...
def _decode_ima_adpcm(data):
	valpred = 0
	index = 0
	out = bytearray()
	for byte in data:
		for delta in (byte >> 4, byte & 0x0F):
			...
			index += _IMA_INDEX_TABLE[delta & 7]
			index = max(0, min(88, index))
			out.extend(struct.pack("<h", valpred))
	return bytes(out)
...
#	  - instance.OriginalAuthorName: The name of the original author in str
#	  - instance.EditorAuthorName: The name of the (previous?) editing author in str
#	  - instance.Username: The username in str
...
class PPM:
	...
	def Read(self, data, DecodeThumbnail=False, ReadFrames=True, ReadSound=False):#Load: (thumbnail, frames, sound)
		if data[:4] != b"PARA" or len(data) <= 0x6a0:
			return False
		...
		self.EditorAuthorID = data[0x5E:0x66][::-1].hex().upper()#the last user to save the file
		
		self.OriginalFilenameC = data[0x66:0x78]#compressed
		self.CurrentFilenameC = data[0x78:0x8a]#compressed
		self.OriginalFilename = "%s_%s_%s.ppm" % (self.OriginalFilenameC[:3].hex().upper(), self.OriginalFilenameC[3:-2].decode("ascii", "replace"), str(AscDec(self.OriginalFilenameC[-2:], True)).zfill(3))
		self.CurrentFilename = "%s_%s_%s.ppm" % (self.CurrentFilenameC[:3].hex().upper(), self.CurrentFilenameC[3:-2].decode("ascii", "replace"), str(AscDec(self.CurrentFilenameC[-2:], True)).zfill(3))
		
		self.PreviousEditAuthorID = data[0x8a:0x92][::-1].hex().upper()#don't know what this really is
		...
		self.Looped = data[0x06A6] >> 1 & 0x01 == 1#Unverified?
		...
		self.SFXUsage = [(i&0x1!=0, i&0x2!=0, i&0x4!=0) for i in data[AudioOffset:AudioOffset+self.FrameCount]]#SFXUsage[frame] = (sfx1, sfx2, sfx2) shere sfxX is either 0 or 1
		...
		if self.forced_speed == None:
			self.Framespeed    = 8 - data[AddPadding(AudioOffset+self.FrameCount, 4) + 16]
		else:
			self.Framespeed = self.forced_speed
		self.BGMFramespeed = 8 - data[AddPadding(AudioOffset+self.FrameCount, 4) + 17]#framespeed when the bgm was recorded
...
##            self.BGMFramespeed = self.forced_speed
				...
				for tx in range(8):
					for y in range(8):
						for x in range(0,8,2):
							...
							byte = self.RawThumbnail[(ty*512+tx*64+y*8+x)//2]
	...
	def GetSound(self, index, outputpath=None):#index 0 is BGM. 1, 2 and 3 is SFX. if outputpath is None, the raw mono PCM @ 8184Hz is returned instead
		if self.Loaded[2]:
			if self.SoundData[index]:
				#reverse nibbles:
				data = []
				for i in self.SoundData[index]:
					data.append((i&0xF)<< 4 | (i>>4))
				data = bytes(data)
				
				#4bit ADPCM decode
				decoded = _decode_ima_adpcm(data)
	...
	def ExtractFrame(self, data, offset, PrevFrame=None):
		...
		Encoding = [[], []]
		#Frame = [[[False for _ in range(192)] for _ in range(256)] for _ in range(2)]
		Frame = np.zeros((2, 256, 192), dtype=np.bool_)
		
		#Read tags:
		NewFrame = data[offset] & 0x80 != 0
...
class TMB:
	...
	def Read(self, data, DecodeThumbnail=False):
		...
		#self.OriginalAuthorName = u"".join(chr(AscDec(data[0x14+i*2:0x14+i*2+2], True)) for i in range(11)).split(u"\0")[0]
		#self.EditorAuthorName = u"".join(chr(AscDec(data[0x2A+i*2:0x2A+i*2+2], True)) for i in range(11)).split(u"\0")[0]
		#self.Username = u"".join(chr(AscDec(data[0x40+i*2:0x40+i*2+2], True)) for i in range(11)).split(u"\0")[0]
		self.OriginalAuthorName = data[0x14:0x2A].decode("UTF-16LE").split(u"\0")[0]
		self.EditorAuthorName = data[0x2A:0x40].decode("UTF-16LE").split(u"\0")[0]
		self.Username = data[0x40:0x56].decode("UTF-16LE").split(u"\0")[0]
		...
		self.OriginalFilenameC = data[0x66:0x78]#compressed
		self.CurrentFilenameC = data[0x78:0x8a]#compressed
		self.OriginalFilename = "%s_%s_%s.tmb" % (self.OriginalFilenameC[:3].hex().upper(), self.OriginalFilenameC[3:-2].decode("ascii", "replace"), str(AscDec(self.OriginalFilenameC[-2:], True)).zfill(3))
		self.CurrentFilename = "%s_%s_%s.tmb" % (self.CurrentFilenameC[:3].hex().upper(), self.CurrentFilenameC[3:-2].decode("ascii", "replace"), str(AscDec(self.CurrentFilenameC[-2:], True)).zfill(3))
	...
	def Pack(self, ppm=None):#not implented
		...
		out = [b"PARA",#magic
			   ...
			   b"\x24\x00",#unknown
			   bytes((int(bool(self.Locked)),)), b"\0",#locked
			   DecAsc(self.ThumbnailFrameIndex, 2, True),#which frame is in the thumbnnail
			   self.OriginalAuthorName.encode("UTF-16LE") + b"\0\0"*(11-len(self.OriginalAuthorName)),#Original Author Name
			   self.EditorAuthorName.encode("UTF-16LE") + b"\0\0"*(11-len(self.EditorAuthorName)),#Editor Author Name
			   self.Username.encode("UTF-16LE") + b"\0\0"*(11-len(self.Username)),#Username
			   bytes.fromhex(self.OriginalAuthorID)[::-1],#OriginalAuthorID
			   bytes.fromhex(self.EditorAuthorID)[::-1],#EditorAuthorID
			   self.OriginalFilenameC,#OriginalFilename
			   self.CurrentFilenameC,#CurrentFilename
			   bytes.fromhex(self.PreviousEditAuthorID)[::-1],#EditorAuthorID
...
def get_metadata(flipnote, filetype=None):
	if filetype is None:
		filetype = "ppm" if isinstance(flipnote, PPM) else "tmb"
...
if __name__ == '__main__':
	...
	if sys.argv[1] == "-t":
		...
		meta = get_metadata(flipnote, filetype)
		newline = "\n"
		if sys.platform in ("win32", "cygwin"): newline = "\r\n"
		elif sys.platform in ("darwin"): newline = "\r"

		metadata_text = newline.join(f"{key}\t{value}" for key, value in meta.items())
		print(metadata_text)
		
		if len(sys.argv) >= 4:
			with open(sys.argv[3], "w", encoding="UTF-8", newline="") as f:
				f.write(metadata_text + newline)
	elif sys.argv[1] == "-oa":
